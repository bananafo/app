...
contract BoostedLiquidityDistributor is Initializable, Ownable {
    ...
    /// @notice Balance of the contract after boosts.
    ...
    function deposit(uint256 amount) external virtual {
        ...
        uint256 userDeposit = _userDeposit[msg.sender];
        ...
        _userDeposit[msg.sender] = userDeposit;
        totalDeposit = totalDeposit + amount;

        _updateBalances(msg.sender, userDeposit, totalDeposit);
    ...
    function withdraw(uint256 amount) external virtual {
        ...
        if (amount > userDeposit) revert InsufficientStakeBalance();
    ...
    function withdrawAndClaim() external virtual returns (uint256 withdrawAmount, uint256 unclaimedRewards) {
        ...
        withdrawAmount = _userDeposit[msg.sender];
    ...
    function _updateBalances(address acccount, uint256 userDeposit, uint256 _totalDeposit) internal virtual {
        ...
        uint256 workingBalance = userDeposit * BOOSTLESS_PRODUCTION / 100;
        if (totalPower > 0) {
            uint256 powerBalance = boostToken.balanceOf(acccount);
            workingBalance += (_totalDeposit * powerBalance / totalPower) * (100 - BOOSTLESS_PRODUCTION) / 100;
        }
        workingBalance = Math.min(userDeposit, workingBalance);


        // Update boosted balances
        uint256 lastUserBalance = _userBalance[acccount];
        _userBalance[acccount] = workingBalance;
    ...
    function _distributeRewards(address acccount) internal virtual returns (uint256) {
        uint256 userBalance = _userBalance[acccount];
        if (userBalance <= 0) return 0;

        _updateRewardsdistribution();

        // Compute undistributed rewards from the delta in rewardsRate since the user deposited
        uint256 undistributedRewards = (userBalance * (_rewardsRate - _userRatedRewards[acccount])) / PRECISION;
